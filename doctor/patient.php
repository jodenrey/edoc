...
<body>
    ...
    // Check if patientid exists in the URL
    if (isset($_GET['patientid']) && isset($_GET['action']) && $_GET['action'] == 'done') {
        $patientid = $_GET['patientid'];

        // Fetch patient information based on patientid
        $sqlmain = "SELECT * FROM patient WHERE pid = '$patientid'";
        $result = $database->query($sqlmain);

        if ($result->num_rows > 0) {
            $patientData = $result->fetch_assoc();
            // Now add this patient data to the table for view
            // You can create or use your existing table here to display this patient info
        }
    }
    ?>
    <div class="container">
